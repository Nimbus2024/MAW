#!/usr/bin/env bash
# E6: alpha 对数网格扫描 (margin<->accuracy 标定)。
# 每个 alpha: 训练 simNPO (固定超参, --grad_log) -> 逐 epoch forget 集评估 (--dump_details)
# -> 实体级 Δ 统计 (exp.diagnosis.stats)。运行清单写入 results/_analysis/e6_runs.tsv。
#
# 用法 (code/ 内, tmux):
#   ./scripts/run_mech_e6_sweep.sh
#   ALPHAS="0.0667 1 15" EPOCHS=3 ./scripts/run_mech_e6_sweep.sh
set -euo pipefail
...
RUNS_TSV="${ANALYSIS_DIR}/e6_runs.tsv"
if [ ! -f "${RUNS_TSV}" ]; then
  printf "run_dir\talpha\toracle_dir\n" > "${RUNS_TSV}"
fi
...
for alpha in ${ALPHAS}; do
  ...
  RUN_DIR="${RESULTS_ROOT}/simNPO/${TS}"
  mkdir -p "${RUN_DIR}/logs/tensorboard" "${RUN_DIR}/config" "${RUN_DIR}/diagnosis"
  echo "== [$(date +%H:%M:%S)] train alpha=${alpha} -> ${RUN_DIR}"
  (
    cd "${CODE_ROOT}"
    "${PYTHON}" -m exp.unlearn.simNPO \
      ...
      --lora_r "${LORA_R}" --lora_alpha "${LORA_A}" --grad_log
  ...
  printf "%s\t%s\t%s\n" "${RUN_DIR}" "${alpha}" "${ORACLE_DIR}" >> "${RUNS_TSV}"
  echo "== [$(date +%H:%M:%S)] alpha=${alpha} done"
done

echo "== [$(date +%H:%M:%S)] sweep done. summary: ${RUNS_TSV}"
